...
-- View consultar histórico de inspeção
create view historico_qualidade as
select 
    i.id_inspecao,
    i.data_inspecao,
...
-- Procedure registrar nova inspeção
DELIMITER $$
create procedure registrar_inspecao(
    in p_id_produto int,
    in p_resultado_inspecao varchar(100)
)
begin
    insert into inspecao_qualidade (resultado_inspecao, id_produto)
    values (p_resultado_inspecao, p_id_produto);
    
    select 'Inspeção registrada com sucesso' as status;
end $$
DELIMITER ;

-- Consultar produtos reprovados na inspeção
select 
    p.id_produto,
    p.descricao,
    i.data_inspecao,
...
from inspecao_qualidade i
join produto p on i.id_produto = p.id_produto
where i.resultado_inspecao like '%reprovado%';